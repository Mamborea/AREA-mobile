...
describe('Register Page Logic', () => {
  ...
  it('should validate email format strictly', () => {
    const isEmailValid = (email: string) => {
      return email.length <= 254 &&
        /^[a-zA-Z0-9._%+-]+@([a-zA-Z0-9]+(-[a-zA-Z0-9]+)*\.)+[a-zA-Z]{2,63}$/.test(email);
    };
  ...
  it('should require all registration fields', () => {
    const areAllFieldsFilled = (name: string, email: string, password: string, confirmPassword: string) => {
      return name.length > 0 && email.length > 0 && password.length > 0 && confirmPassword.length > 0;
    };
    
    expect(areAllFieldsFilled('John', 'john@example.com', 'Pass123!', 'Pass123!')).toBe(true);
    expect(areAllFieldsFilled('', 'john@example.com', 'Pass123!', 'Pass123!')).toBe(false);
